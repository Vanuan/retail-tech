  ...
  implements BABYLON.ICameraInput<BABYLON.UniversalCamera>
{
  ...
  private _onPointerMove(pi: BABYLON.PointerInfo) {
    ...
    if (
      this._pointerMap.size === 2 &&
      pi.event.pointerType === "touch"
    ) {
  ...
  private _onPointerWheel(pi: BABYLON.PointerInfo) {
    ...
    if (delta < 0) {
      ...
      if (this._focusTarget) {
        ...
        // // Compute angles to keep looking at this target from new position
        ...
        // // // Smoothly blend to desired angles (approximate "Zoom At" effect)
        // // const blend = 0.2;
        // // this.camera.rotation.y += (desiredYaw - this.camera.rotation.y) * blend;
        // // this.camera.rotation.x +=
        // //   (desiredPitch - this.camera.rotation.x) * blend;

        // // Also drift X slightly to center the target
        // this.camera.position.x +=
        //   (target.x - this.camera.position.x) * (blend * 0.5);
      }
    } else {
      // // ZOOM OUT
      // this._focusTarget = null;

      // // Re-establish a neutral forward look
      // const forwardTarget = this.camera.position.add(
      //   new BABYLON.Vector3(0, 0, -1)
      // );
      // this.camera.setTarget(forwardTarget);
